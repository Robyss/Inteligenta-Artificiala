...
class Graph: #graful problemei
    def __init__(self, nume_fisier):
        ...
        self.__class__.N=int(listaInfoFisier[0])    # numar de cofetari
        self.__class__.M=int(listaInfoFisier[1])    # 
        self.__class__.cost_mutare = int(listaInfoFisier[2])
        self.__class__.nr_barca = int(listaInfoFisier[3])
        self.__class__.K = int(listaInfoFisier[4])  # prajituri produse de bucatar in barca
        self.__class__.W = int(listaInfoFisier[5])  # munca efectuata pe malul final
...
#     1+C/NR_COFETARI_BARCA (C
# împărțit la câți cofetari sunt în barcă) când sunt doar cofetari în barcă (și deci fac prăjituri) și egal cu
# NR_COFETARI_BARCA*C+COST_FLAMANZI când transportul e mixt (cofetari alături de omuleți
# flămânzi. Parametrul COST_FLAMANZI este NR_FLAMANZI când barca merge de pe malul inițial spre final
# si 2*NR_FLAMANZI la întoarcere. NR_FLAMANZI este numărul de omuleți flămânzi din barcă. Dacă barca
# mută doar omuleți flămânzi (numărul de cofetari e nul), costul întregului transport va fi 1.
    ...
    def calculeaza_h(self, infoNod, tip_euristica="euristica banala"):
        t1 = time.time()    # cu toate ca nu face mai nimic pe calculul asta, trebuie puncte

        if tip_euristica=="euristica banala":
            
            if not infoNod[0]==infoNod[1]==infoNod[2]==0:
                timp_calcul_euristica += (time.time() - t1)
                return 1
            timp_calcul_euristica += (time.time() - t1)
            return 0            
        else:
...
def a_star(gr, nrSolutiiCautate, tip_euristica):
    #in coada vom avea doar noduri de tip NodParcurgere (nodurile din arborele de parcurgere)
    import heapq  
    radacina = NodParcurgere(gr.start, None, 0, gr.calculeaza_h(gr.start))
    c = []
    heapq.heappush(c, (radacina.f, radacina))   # Introducem in coada de prioritati
    
    
    while len(c):
        nodCurent=heapq.heappop(0)
        ...
        for s in lSuccesori:
            heapq.heappush(c, (s.f, s)) # introducem succesorii in coada
